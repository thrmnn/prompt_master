    ...
    for domain, domain_cases in sorted(by_domain.items()):
        ...
        for c in domain_cases:
            ...
            if judge and judge.get("avg_score", 0) > 0:
                lines.append(
                    f"           Judge: {judge['avg_score']:.1f}/5.0  ({judge.get('summary', '')})"
                )

            # Show failed checks
            if structural:
                failed = [ch for ch in structural.get("checks", []) if not ch.get("passed")]
                for ch in failed:
...
def format_comparison(
    report_a: dict, report_b: dict, label_a: str = "A", label_b: str = "B"
) -> str:
